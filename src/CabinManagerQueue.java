...
public class CabinManagerQueue {
    static Queue queue = new Queue();
    static int fullCabinsCount = 0;
    ...
    /**
     Prompts cabin number they want to add customers from user.  If cabins are not full add customers to it or
     if cabins are full add customers to queue.
     This loops till user enters 0 to stop.
     parameters- array of 12 Cabin Objects
     Return- None
     */

    public static void addCustomers (Cabin [] pCabinNo) {
        Scanner input = new Scanner(System.in);
        while (true){
            try{
                ...
                if ( cabNo <= 12 && cabNo > 0){//Checking if the prompted cabin number is within the cabin range to handle ArrayOutofBoundry exception
                    if (fullCabinsCount == 12) {
                        addToQueue();
                    }
                    else if ( !pCabinNo[cabNo-1].isCabinFull()){     //Checking if each cabin is full of 3 passengers before adding new passengers
    ...
    /**
     Prompts first name from user and deletes the passenger from cabin he is in. Adds passengers from queue to empty slot
     parameters- Array of 12 Cabin Objects
     Return- None
     */
                ...
                if (pCabinNo[i].getPassengers()[j] != null) {
                    if (pCabinNo[i].getPassengers()[j].getFName().equalsIgnoreCase(cusName)) {      //Comparing first names
                        ...
                        if (!queue.isEmpty()){             //Checking if queue is not empty
                            pCabinNo[i].addPassengersToArray(queue.dequeue());         //Adding passenger from queue to cabin
                            pCabinNo[(i)].setAvailability("Occupied");       //Changing initialized Empty status
                            System.out.println("The empty slot in cabin " + (i+1) + " got successfully filled");
                        }
                    }
                }
            }
    ...
    public static void expenses (Cabin [] pCabinNo) {
        ...
        for (int i = 0; i < pCabinNo.length; i++ ) {
            if (!pCabinNo[i].getAvailability().equals("Empty")){
                for ( int j = 0; j < 3; j++ ) {
                    if (pCabinNo[i].getPassengers()[j] != null) {
                        System.out.println("       " + pCabinNo[i].getPassengers()[j].toString() );       //Displaying expenses per passenger
                    }
                }
            }
        }
    ...
    /**
     Calls enqueue method from class Queue and adds passengers to waiting list
     parameters- None
     Return- None
     */
    public static void addToQueue () {
        if (!queue.isFull()){           //Check if the queue is full
            ...
            queue.enqueue(passengerDetails);       //Add passenger to queue

            System.out.println("\nYour details got added to the waiting list");
        }
        else {      //If queue is full
            System.out.println("Sorry!! Waiting List is Full");
        }
    }
}
